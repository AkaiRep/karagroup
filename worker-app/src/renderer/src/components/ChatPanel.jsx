...
export default function ChatPanel({ orderId, onClose }) {
  ...
  useEffect(() => {
    if (!orderId) return
    setLoading(true)
    getMessages(orderId).then(setMessages).finally(() => setLoading(false))
    markRead(orderId)
  ...
  return (
    <div className="flex flex-col h-full bg-[#1a1f2e]">
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-700/50 flex-shrink-0">
        <div className="text-sm font-medium text-slate-300">Чат с администратором</div>
        {onClose && (
          ...
          return (
            <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'} group`}>
              <div className="max-w-[80%]">
                {!isMe && (
                  <div className="text-xs text-slate-400 mb-0.5 px-1">{msg.sender?.username || 'Admin'}</div>
                )}
                <div className={`rounded-xl ${isMe ? 'bg-brand-500 text-white' : 'bg-[#0f1117] text-slate-200'} ${msg.image_url ? 'p-1.5' : 'px-3 py-2'}`}>
                  {msg.image_url ? (
                    <ImageMessage
                      msg={msg}
                      isMe={isMe}
      ...
      <form onSubmit={handleSend} onPaste={handlePaste} className="px-3 py-3 border-t border-slate-700/50 flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          />
          <input
            className="flex-1 bg-[#0f1117] border border-slate-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-brand-500"
            value={text}
...
function ImageMessage({ msg, isMe, copied, onCopy, onDownload }) {
  ...
  return (
    <div className="relative group/img">
      ...
      />
      <div className="absolute inset-0 bg-black/60 opacity-0 group-hover/img:opacity-100 transition-opacity rounded-lg flex items-center justify-center gap-2">
        <button onClick={onCopy} className="bg-white/20 hover:bg-white/30 text-white text-xs px-2 py-1 rounded transition-colors">
          {copied ? '✓' : 'Копировать'}
        </button>
        <button onClick={onDownload} className="bg-white/20 hover:bg-white/30 text-white text-xs px-2 py-1 rounded transition-colors">
          Скачать
        </button>
      </div>
