...
function Card({ title, icon, children }) {
  return (
    <div className="bg-surface rounded-xl border border-border/50 overflow-hidden">
      <div className="px-5 py-3 border-b border-border/50 bg-base flex items-center gap-2">
        {icon && <span>{icon}</span>}
    ...
    <div className="p-8 max-w-4xl">
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          onClick={() => load(true)}
          disabled={refreshing}
          className="flex items-center gap-2 px-4 py-2 bg-surface border border-border hover:border-green-500/40 rounded-xl text-sm transition-colors disabled:opacity-50"
        >
      ...
      {loading && (
        <div className="space-y-4">
          {[1, 2, 3, 4].map(i => (
            <div key={i} className="bg-surface rounded-xl h-32 animate-pulse border border-border/50" />
          ))}
        </div>
      )}
      ...
      {data && !loading && (
        <div className="space-y-4">
          ...
          {data.history && data.history.length <= 1 && (
            <div className="bg-surface rounded-xl border border-border/50 p-6 text-center text-slate-500 text-sm">
