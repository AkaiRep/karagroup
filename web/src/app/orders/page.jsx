...
function OrdersContent() {
  ...
  const resultLabel = () => {
    if (checkResult === 'paid') return t('orders.payConfirmed')
    if (checkResult === 'paid_multi') return t('orders.payConfirmed')
    if (checkResult === 'none') return t('orders.noPayments')
    if (checkResult === 'error') return t('orders.payError')
    return t('orders.checkPayment')
  }
  ...
  if (!loading && user && !user.telegram_id) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center">
        ...
        <h2 className="text-xl font-semibold mb-2">{t('orders.noTg')}</h2>
        <p className="text-slate-400 text-sm mb-6">{t('orders.noTgMsg')}</p>
  ...
  if (fetchError) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-8 text-center py-20">
        <p className="text-slate-400">{t('orders.loadError')}</p>
      </div>
    )
  }
    ...
    <div className="max-w-2xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        ...
        >
          ...
          {checking ? t('orders.checking') : resultLabel()}
        </button>
      </div>
      ...
      {orders.length === 0 ? (
        <div className="text-center py-20">
          ...
          <p className="text-slate-400">{t('orders.empty')}</p>
        </div>
      ) : (
